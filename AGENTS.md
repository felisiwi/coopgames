...
1. **No TURN server — ADJUST (softer than stated).** The shipped PeerJS `DEFAULT_CONFIG` includes a TURN pair (`turn:eu-0.turn.peerjs.com:3478`, `turn:us-0.turn.peerjs.com:3478`, user `peerjs`/`peerjsp`) alongside Google STUN — the peerjs.com API docs still say STUN-only, but the code disagrees. So v1 gets a best-effort relay for free with no work. Resolution: use PeerJS defaults, no custom ICE config. Stage 3 agent must verify by grepping the vendored peerjs bundle for `turn.peerjs.com` and paste the match as evidence. Two-different-networks test stays mandatory for stage 3 done (phone hotspot vs home wifi is enough).
