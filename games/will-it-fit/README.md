...
## Build status — stage 1 of 5

The [build plan](DESIGN.md#build-plan) is a tracer bullet. **Stage 1 is done:** the ballistic
stream, the CA, the flight→settle handoff, vessel generation, filling, overflow and the pool
economy all work, solo, with no networking.
...
No browser check yet, and no hub launch — `games/README.md` requires testing through the hub
before sign-off, since the hub loads `game.js` from a different URL than the solo entry. The
pool economy numbers (spill cost, source volume, the doubling refill schedule) are guesses
